...
func _on_body_exited(body: Node2D) -> void:
	...
	if estado == EstadoLoja.MENU:
		fechar_menu()
	elif estado == EstadoLoja.CONVERSA:
...
func abrir_conversa() -> void:
	...
	var falas: Array[String] = [
		"Salve... chega mais, parceiro.",
		"quer dar uma melhorada nesses seus pontos?",
		"aqui eu troco moedas por score...",
		"mas fica esperto: nada vem de graca nesse lugar.",
		"por apenas 10 moedas, voce leva 1000 pontos.",
		"vai querer virar uma lenda?"
	]
	...
	if not is_inside_tree() or not jogador_na_area:
		estado = EstadoLoja.ESPERANDO
		bloqueado = false
		return
	...
	if not jogador_na_area or estado != EstadoLoja.CONVERSA:
		estado = EstadoLoja.ESPERANDO
		bloqueado = false
		return
...
func criar_menu() -> void:
	...
	# =====================================================
	# ICONE / DESTAQUE DO SCORE
	# =====================================================

	icone_score = criar_label(
		"+1000 SCORE",
		22,
		Color("#F0A0FF")
	)

	icone_score.position = Vector2(
		0.0,
		82.0
	)

	icone_score.size = Vector2(
		600.0,
		55.0
	)

	icone_score.horizontal_alignment = (
		HORIZONTAL_ALIGNMENT_CENTER
	)

	icone_score.vertical_alignment = (
		VERTICAL_ALIGNMENT_CENTER
	)

	icone_score.mouse_filter = (
		Control.MOUSE_FILTER_IGNORE
	)

	painel.add_child(icone_score)
	...
	score_atual = criar_label(
		"COMPRAR",
		10,
		Color("#F5D56A")
	)

	score_atual.position = Vector2(
		0.0,
		226.0
	)

	score_atual.size = Vector2(
		600.0,
		22.0
	)

	score_atual.horizontal_alignment = (
		HORIZONTAL_ALIGNMENT_CENTER
	)

	painel.add_child(score_atual)
	...
	voltar = Button.new()
	voltar.text = "VOLTAR"
	voltar.position = Vector2(45.0, 294.0)
	...
	comprar.position = Vector2(325.0, 294.0)
...
func atualizar_dados() -> void:
	...
	score_atual.text = "SCORE ATUAL: %d" % Globals.score
...
func processar_menu(event: InputEvent) -> void:
	...
	# -----------------------------------------------------
	# ESC
	# Fecha a loja sem abrir o menu de pausa.
	# -----------------------------------------------------
...
func comprar_score() -> void:
	...
	if Globals.coins >= PRECO_SCORE:
		...
		resultado.text = "NEGOCIO FECHADO! +1000 SCORE"
	...
	# =====================================================
		...
		resultado.text = "FALTAM %d MOEDAS... VOLTA COM MAIS" % faltam
